...
def loc_shift_gate(num_qubits: int, direction: str) -> Gate:
    """Create quantum gates for a left or right index shift.
    ...
    if direction == "l":
        ...
        raise NameError(
            f"Direction variable ('{direction}') must be either 'l' or 'r'."
        )
...
class BandedCirculantOracle(SparseOracle):
    ...
    def location_oracle(self) -> QuantumCircuit:
        ...
        loc_oracle_circuit.append(
            c_l_shift_gate, [0] + list(range(2, loc_reg_num_qubits + 2))
        )
        loc_oracle_circuit.append(
            c_r_shift_gate, [1] + list(range(2, loc_reg_num_qubits + 2))
        )
